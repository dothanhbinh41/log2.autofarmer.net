...
namespace LogJson.AutoFarmer.Controllers
{
    ...
    public class ScreenLogController : ControllerBase
    {
        ...
        {
            var data = await autoFarmerDistributeCache.GetAsync<List<XmlLogDto>>(request.AndroidId);
            data ??= new List<XmlLogDto>();
            var log = mapper.Map<XmlLogDto>(request);
            log.CreationDate = DateTime.UtcNow;
            data.Add(log);
            await autoFarmerDistributeCache.SetAsync(request.AndroidId, data, new Microsoft.Extensions.Caching.Distributed.DistributedCacheEntryOptions { AbsoluteExpirationRelativeToNow = TimeSpan.FromDays(1) });
            return true;
        }
        ...
        public async Task<PagedResult<XmlLogDto>> GetXmlLogAsync([FromBody] GetXmlLogRequestDto request)
        {
            var data = await autoFarmerDistributeCache.GetAsync<List<XmlLogDto>>(request.AndroidId);
            data ??= new List<XmlLogDto>();
            return new PagedResult<XmlLogDto> { Count = data.Count, Items = data.Skip((request.Page - 1) * request.Limit).Take(request.Limit).ToList() };
        }
    }
}
